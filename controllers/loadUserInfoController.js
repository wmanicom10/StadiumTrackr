...
const handleLoadUserInfo = async (req, res) => {
    ...
    try {
        ...
        const [userStadiums] = await db.execute('SELECT MAX(added_on) AS added_on, user_stadiums.stadium_id, stadium_name, city, state, image FROM user_stadiums JOIN stadiums ON user_stadiums.stadium_id = stadiums.stadium_id WHERE user_id = ? GROUP BY stadium_name, user_stadiums.stadium_id, city, state, image ORDER BY added_on DESC', [userId]);
        ...
        const [wishlistItems] = await db.execute('SELECT added_on, user_wishlist_stadiums.stadium_id, stadium_name, city, state, image FROM user_wishlist_stadiums JOIN stadiums ON user_wishlist_stadiums.stadium_id = stadiums.stadium_id WHERE user_id = ? ORDER BY added_on DESC', [userId]);
